...
describe("PasswordService", () => {
  describe("hash_md5", () => {
    it("returns an MD5 hash for a valid input", async () => {
      ...
      const hashed_input = await CryptoService.hash_md5(input);
    ...
    it("returns different MD5 hashes for different inputs", async () => {
      const input1 = "input1";
      const input2 = "input2";
      const hashed_input1 = await CryptoService.hash_md5(input1);
      const hashed_input2 = await CryptoService.hash_md5(input2);
      expect(hashed_input1).not.toBe(hashed_input2);
    });

    it("returns the same MD5 hash for the same input", async () => {
      const input = "sameInput";
      const hashed_input1 = await CryptoService.hash_md5(input);
      const hashed_input2 = await CryptoService.hash_md5(input);
  ...
  describe("hash", () => {
    it("returns a hashed string for a valid password", async () => {
      ...
      const hashed_password = await CryptoService.hash(password);
      expect(hashed_password).toMatch(/^\$2[ayb]\$.{56}$/);
    });

    it("returns different hashes for the same password", async () => {
      const password = "validPassword";
      const hashed_password_1 = await CryptoService.hash(password);
      const hashed_password_2 = await CryptoService.hash(password);
  ...
  describe("compare", () => {
    it("returns true for matching password and hash", async () => {
      const password = "validPassword";
      const hashed_password = await CryptoService.hash(password);
      const is_match = await CryptoService.compare(password, hashed_password);
    ...
    it("returns false for non-matching password and hash", async () => {
      const password = "validPassword";
      const hashed_password = await CryptoService.hash(password);
      const is_match = await CryptoService.compare(
        "invalidPassword",
        hashed_password,
      );
  ...
  describe("generate_challenge", () => {
    it("returns a string of correct length", () => {
      const result = CryptoService.generate_challenge();
    ...
    it("returns a different string each time it is called", () => {
      const result1 = CryptoService.generate_challenge();
      const result2 = CryptoService.generate_challenge();
